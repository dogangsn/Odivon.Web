...
export default function Home() {
 ...
 const selected = ['odivon-fatura-pro','odivon-farm','odivon-school'].map(id=>products.find(product=>product.id===id)!);
 return <main id="main">
 ...
 <section className="home-products wrap"><div className="home-products-heading"><div><span className="section-label">{t.productLabel}</span><h2>{u.catalogueLink}</h2><p>{locale==='tr'?'Finans, tarım, eğitim ve daha fazlası. Her çözüm, kendi alanındaki gerçek bir iş için tasarlandı.':'Finance, agriculture, education and more. Every solution is designed around a real task.'}</p></div><a className="text-link" href="/urunler">{u.allProducts}<ArrowUpRight size={19}/></a></div><div className="home-product-grid">{selected.map((product,index)=><a className="home-product" href={`/urunler#${product.id}`} key={product.id} data-product={product.symbol}><span className="home-product-number">0{index+1} / {product.category[locale]}</span><h3>{product.name}</h3><p>{product.description[locale]}</p><span className="home-product-link">{u.details}<ArrowUpRight size={18}/></span></a>)}</div></section>
