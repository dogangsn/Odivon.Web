...
export function SiteShell({children}:{children:ReactNode}) {
 ...
 <header className="header wrap">{brand}<nav aria-label={t.nav}><a href="/" aria-current={pathname==='/'?'page':undefined}>{u.home}</a><a href="/urunler" aria-current={pathname==='/urunler'?'page':undefined}>{t.products}</a><a href="/#odivon">{t.about}</a><a href="/#iletisim">{t.contact}</a></nav><div className="preference-controls">
 ...
 <footer className="wrap"><div className="footer-top">{brand}<p>{t.footer}</p><a href="#main">{t.top}<ArrowUpRight size={17}/></a></div><div className="footer-bottom"><span>© {new Date().getFullYear()} Odivon</span><a href="mailto:info@odivon.com">info@odivon.com</a><span>{u.productFooter}</span></div></footer>
