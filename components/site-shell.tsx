...
export function SiteShell({children}:{children:ReactNode}) {
 ...
 const brand=<Link href="/" className="brand" aria-label={t.home}><span className="brand-symbol" aria-hidden="true"/>odivon<span className="brand-dot" aria-hidden="true">.</span></Link>;
 return <Preferences.Provider value={{locale,theme}}>
 <a className="skip-link" href="#main">{t.skip}</a>
 <header className="header wrap">{brand}<nav aria-label={t.nav}><Link href="/" aria-current={pathname==='/'?'page':undefined}>{u.home}</Link><Link href="/urunler" aria-current={pathname==='/urunler'?'page':undefined}>{t.products}</Link><Link href="/#odivon">{t.about}</Link></nav><div className="preference-controls">
